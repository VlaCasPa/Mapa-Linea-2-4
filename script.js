...
function formatearDias(dias) {
    ...
    if (d === "culminado" || d === "culminada") return "Obra Culminada";
...
function aplicarFiltros() {
    ...
    marcadoresGuardados.forEach(obj => {
        ...
        // Coloración dinámica al activar filtros puntuales
            ...
            if (item.Latitud && item.Longitud && item.ID) {
                ...
                let comObra = item.Aut_Obra_Comentarios ? `
                    <details class="popup-details">
                        <summary class="popup-summary">Ver comentario de obra</summary>
                        <div class="popup-comment-text">${item.Aut_Obra_Comentarios}</div>
                    </details>` : '';

                let comDesvio = item.Aut_Desvio_Comentarios ? `
                    <details class="popup-details">
                        <summary class="popup-summary">Ver comentario de desvío</summary>
                ...
                if (!isNaN(lat) && !isNaN(lon)) {
                    ...
                    // Condición preventiva (< 4 meses / 120 días vigente)
                    let venceMenor4Meses = (
                    ...
                    let marker = L.circleMarker([lat, lon], {
                        ...
                        opacity: 1,
                        fillOpacity: 0.8,
                        className: venceMenor4Meses ? 'brillo-preventivo' : ''
                    });
                    ...
                    marcadoresGuardados.push({
                        ...
                        esMenor4Meses: venceMenor4Meses
                    });
                }
            }
        });

        // Listeners de botones ID
...
document.getElementById('btn-whatsapp').addEventListener('click', () => {
    ...
    window.datosGlobales.forEach(item => {
        if (item.ID && item.Latitud) {
            ...
            if (esCulminado) culminadas.push(item.ID);
        }
    });
    ...
    texto += `🔴 *VENCIDAS (${vencidos.length}):*\n${vencidos.length > 0 ? vencidos.join(', ') : 'Ninguna'}\n\n`;
    texto += `🟠 *CRÍTICAS <29 DÍAS (${criticos.length}):*\n${criticos.length > 0 ? criticos.join(', ') : 'Ninguna'}\n\n`;
    texto += `🟡 *POR VENCER < 4 MESES (${menor4Meses.length}):*\n${menor4Meses.length > 0 ? menor4Meses.join(', ') : 'Ninguna'}\n\n`;
    texto += `🟣 *EN TRÁMITE (${tramite.length}):*\n${tramite.length > 0 ? tramite.join(', ') : 'Ninguna'}\n\n`;
    texto += `🟢 *CULMINADAS (${culminadas.length}):*\n${culminadas.length > 0 ? culminadas.join(', ') : 'Ninguna'}\n\n`;
    texto += `⚖️ *LEY N° 31955 (${ley31955.length}):*\n${ley31955.length > 0 ? ley31955.join(', ') : 'Ninguna'}\n\n`;
    texto += `🔗 *Visor Web:* https://vlacaspa.github.io/Mapa-Linea-2-4/`;

    // Copiado compatible y redirección a WhatsApp
    ...
    let urlWhatsapp = `https://api.whatsapp.com/send?text=${encodeURI(texto)}`;
