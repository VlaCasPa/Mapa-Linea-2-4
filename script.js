...
// CORRECCIÓN: Separar la promesa de persistencia de los eventos de clic para garantizar que siempre funcionen
setPersistence(auth, browserLocalPersistence).catch((error) => {
    console.warn("Aviso de Persistencia:", error);
});
...
btnLoginGoogle.addEventListener('click', () => {
    ...
    signInWithPopup(auth, provider).catch(() => {
        mensajeError.innerText = "Error de autenticación. Verifica tus permisos o conexión a internet.";
...
function configurarManejadoresReportes() {
    ...
    if (btnPdf) {
        btnPdf.addEventListener('click', () => {
            ...
            let totalTramitesTotales = window.datosGlobales.length * 2; 
            ...
            // Forzar visualización temporal al frente de la pantalla para evitar que html2canvas capture un lienzo en blanco
            elementoAImprimir.style.position = 'fixed';
            elementoAImprimir.style.left = '0'; 
            elementoAImprimir.style.top = '0';
            elementoAImprimir.style.zIndex = '9999'; 
            
            setTimeout(() => {
                const opciones = {
                    margin: 0,
                    filename: `Reporte_Ejecutivo_L2L4_${fechaActual.getTime()}.pdf`,
                ...
                html2pdf().set(opciones).from(elementoAImprimir).save().then(() => {
                    // Ocultar plantilla nuevamente tras finalizar la descarga
                    elementoAImprimir.style.left = '-9999px'; 
                    elementoAImprimir.style.zIndex = '-10';
                    elementoAImprimir.style.position = 'absolute';
                    
                    btnPdf.innerHTML = "✅ Descargado";
                    setTimeout(() => { btnPdf.innerHTML = tOri; }, 3000);
                });
            }, 150); // El retraso asegura el pintado del DOM
        });
    }
}
